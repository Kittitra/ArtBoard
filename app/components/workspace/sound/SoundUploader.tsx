...
interface Props {
    onReady: (uploadId: string, playbackId: string) => void;
    onClose: () => void;
    onStartUpload?: () => void;
}

const SoundUploader = ({ onReady, onClose, onStartUpload }: Props) => {
    ...
    const handleFile = async (file: File) => {
        setStatus("uploading");
        onStartUpload?.();
        // 1. ขอ upload URL จาก server (ใช้ endpoint เดียวกับวิดีโอได้เลย ไม่ต้องแยก)
        ...
        const poll = async () => {
            ...
            if (data.status === "ready" && data.playbackId) {
                setStatus("ready");
                onReady(uploadId, data.playbackId); // ไม่มี thumbnail สำหรับเสียง
            } else {
    ...
    return (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center gap-4 w-96">
                <h3 className="font-semibold text-gray-700">Upload Sound</h3>
                ...
                {status === "idle" && (
                    <>
                        ...
                        >
                            <span className="text-gray-400 text-sm">Click to select audio file</span>
                        </div>
                        <input
                            ref={inputRef}
                            type="file"
